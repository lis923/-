...
TITLE B站搜“一只靓仔琦” 更多好玩等你发现（版本2022.02.17持续更新）          互助Q群110555071
color 8e
:Begain
CLS
...
ECHO.===============================连接成功后会自动开启主菜单=======================================
adb wait-for-device
ECHO.下次您再打开鸿蒙工具箱时记录会清空，有需要记得拷贝走  > 操作记录.txt
ECHO. >> 操作记录.txt
ECHO. >> 操作记录.txt
GOTO STARTS
:STARTS
...
ECHO.    14       开启巅峰性能模式（-14开启，+14关闭）       =游戏更稳，日常建议关
ECHO.                                                      荣耀9麒麟960：飞车王者更稳
ECHO. 
ECHO.    20        开启系统禁更（-20禁更，+20恢复）           =禁止系统催更新
ECHO. 
...
ECHO.             g12     强制固定屏幕刷新率至96hz       （仅120hz高刷屏可用）
ECHO. 
...
ECHO. 
ECHO.  
ECHO.   备注： 输出结果会导出到result.txt，可自行取走
ECHO. 
ECHO.===============================================================================================
ECHO.
ECHO.  【模式跳转】任何时候/位置输入都可跳转！
...
ECHO.  
ECHO.  
ECHO.当您使用此功能时，请您务必阅读以下内容
ECHO.
ECHO.   您将解开处理器的功耗限制及游戏加速模式下的温度限制
ECHO.   同时禁止能够系统后台唤醒app （使日常更加省电流畅）
ECHO.   若电池功率低或老化无法发挥最佳实力，您可插电保证功率
ECHO.   若温度升起影响内存性能，您可改善散热来保证性能流畅
ECHO.   当温度过高时，系统会强制您关闭游戏和其他后台应用
ECHO.   建议您做好散热措施，高性能的游戏耗电快是正常现象
ECHO.   游戏前请开启【设置】-【应用】-【应用助手】-【游戏加速】
ECHO.   渣机体验明显，但建议游戏后关闭
ECHO.   有的机型带来不好的体验，您可以及时关闭并重启 
ECHO. 
ECHO.以下是执行结果：（success即成功）
...
ECHO.高清通话：采用4G流量替代2G通话，能够传输更高码率的声音
ECHO.
pause
GOTO STARTS2
:043
...
ECHO.  
ECHO.  此方案来源 https://tech.sina.com.cn/digi/2020-03-29/doc-iimxxsth2473542.shtml
ECHO.  不保证一定有效，若无效果则是您手机中settings命令中并无此参数，因此并不会改动任何东西
